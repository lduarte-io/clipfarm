...
4. **Debug harness — `n2harness`** executable target in ClipFarmKit (never ships; not part of the library product, like CFTestSupport): hand-rolled subcommands, one per gate (no new dependencies — argument parsing is by hand). Runs against real dogfood files (read-only, absolute paths) + synthesized fixtures (see PROVISIONAL 1). Subcommands: `fixtures` (generate synthetic media into a workdir), `seams`, `blink`, `rotation`, `hdrseam`, `rebuild`, `frameacc`, `looptest`, `fades`, `exportspike`, `demo` (windowed multi-source assembly playback for Lillian's watch session).
