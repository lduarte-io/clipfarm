...
14. **Footage inbox (added 2026-07-06, Lillian's call — decision D34)**: ClipFarm reads source footage from a dedicated drop folder **outside any cloud-synced path** — default **`~/ClipFarm/Footage/`**, created on first run. The inbox is a **managed working folder, not canonical storage**: it's where video goes to be edited (like any editor's media pool), users keep their master copies elsewhere, and ClipFarm may organize and **delete** files within it; the existing unavailable-source greying is the safety net for files that disappear. Older references in this document to the dogfood folder under `~/Desktop/AdAstra/…` are historical — that path is retired from operative use (it lives on iCloud-synced Desktop, which the 2026-07-06 incident showed strangles bulk file I/O and can evict media to cloud-only). Existing files stay where they are until Lillian moves them into the inbox herself.
15. **Assembly outline (added 2026-07-11, Lillian's call — decision D37)**: the project script's two roles are split. The **matching script** ("what I tried to say") keeps driving string-matched take detection, unchanged. The **assembly outline** — an ordered list of beats, **one outline per project** — is what the Script TOC assembles by: each beat is either a script line or a **free-text intent** matched semantically by the LLM (reusing the ad-hoc tag machinery, same validation). Absent an explicit outline, the default outline is the script lines in order — the scripted-takes wedge case is unchanged. Lands in phases N6–N8. "A different structure is a different project": a **Duplicate Project** action (clones brief/outline/tags with zero re-tagging; Track 1.5, D40) makes that instant. Multiple outlines *within* one project (a third versioning level nested between projects and attempts) was considered and rejected — see D37.
...
- **Merge adjacent clips** — select two consecutive clips on the same source, "merge." Replaces them with one new clip spanning `(min_start, max_end)`. Any non-overlapping gap between the originals — including the ≥2-second silences that ingest treats as clip boundaries — is **folded into the merged range deliberately**: merge is the user telling ClipFarm "the segmentation was wrong, treat this as one take, silence and all." **Merge of overlapping clips is rejected** (the merge operation is undefined for overlapping ranges — split the user's intent into separate create-clip-from-range actions instead). Tags from both clips are union-merged; duplicates dedupe on `(project_id, project_tag_id, category)`.

  **Note on overlapping clips generally**: as of Phase 10a dogfood (2026-05-26), **`create_clip_from_range` allows overlapping ranges on the same source**. The user case is legitimate: same recording can be clipped twice for different purposes (e.g., a 30-second take and a 10-second highlight inside it). Originally the spec rejected overlap blanket; refined to merge-only on usage feedback. Take-detection strategies (Phase 8) treat overlapping clips as independent — minor visual weirdness in the by-source view and in take-grouping output, accepted v0 tradeoff. **Native amendment (2026-07-05, pre-build finding 9, Lillian's call): boundary adjust allows overlap too** — the web implementation rejected any overlap on adjust, which froze deliberately-overlapping clips (neither could be nudged). Overlap is a legal state; only merge rejects overlapping ranges.
...
- **`attempts[id].clips[i].internal_pause_max_sec`** — when non-null, the preview and export **drop any inter-word gap longer than this value entirely** between sub-ranges. Per-attempt-clip, never mutates the base. The resolver produces multiple `(start, end)` sub-ranges from a single attempt-clip when this is set; the seek-on-`ended` trick handles them transparently. v0 ships a single "tighten internal pauses" toggle with a sensible default (e.g. 0.5s); the full per-segment aggressiveness slider is v1 polish layer. (Locked Phase 9 plan review 2026-05-26: spec originally said "collapse down to this value" but the resolver / export semantic is gap-dropped-entirely between sub-ranges — simpler implementation and equivalent listening experience for the dogfood use case. If "preserve a beat of silence" matters later, that's a v1 toggle on top.)
...
On load, ClipFarm checks `schema_version == 1` and refuses (with a clear error pointing at `transcribe.py`) any other version — higher, lower, or missing. A `WhisperTranscript` Pydantic model validates the shape at ingest. If `transcribe.py` ever bumps its schema, ClipFarm needs a matching adapter — but it never silently consumes an unknown shape.
