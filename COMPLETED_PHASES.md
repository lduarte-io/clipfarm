...
**Measurement environment (incident-note rule):** repo, workdir (`~/Library/Caches/ClipFarm-N2Gates`), and footage inbox all live outside iCloud-synced paths; `fileproviderd`/`cloudd` CPU was checked and logged **before every gate** — at or near **0.0% for every measurement run** (per-gate daemon lines in the driver logs; all numbers reproducible via the harness). Gates ran on the **release** build — debug Swift hot loops measured 20–50× slower (fixture rendering ~8 min debug → 58 s release) and release is the shipping configuration.

### Measured gate table (programmatic — PROVISIONAL 2; perceptual confirmation is the watch session; every verdict below is Lillian's to ratify or overturn)

| # | Gate (plan §4/N2) | Leg / material | Numbers (release; daemons ~0%) | Automated verdict |
|---|---|---|---|---|
| 1 | Seam p95 inter-frame gap ≤ 1 frame duration, 20+ non-KF cuts | uniform: synthetic h264A+h264B+hevc1080 (1080p30, bare comp, 23 cross-file seams) | gaps p50 13.7 / p95 28.6 ms; ratio vs minFrameDuration p95 **0.86**; vs measured cadence p95 0.85 | **PASS** |
| | | real inbox: iphone↔preresclip alternating (23 seams, videoComposition) | p50 13.3 / p95 **108.0** / max 126.4 ms; cadence-ratio p95 3.33 | **FAIL** — stalls concentrate at entries into the heavy VFR file |
| | | mixed: preresclip + 4K-HEVC/ProRes/HLG/portrait fixtures (21 seams) | p50 23.1 / p95 **33.3 ms**; minFrameDuration-ratio p95 8.01 (VFR denominator artifact); cadence-ratio p95 1.60 | FAIL as-worded; ~1-frame-at-30fps absolute — borderline VFR-fair |
| | | solo self-splice, iphone (11 same-file seams) | p95 28.7 ms; cadence-ratio p95 **0.86** | **PASS** — the dogfood-profile case is seamless |
| | | solo self-splice, preresclip | p95 **68.4 ms**; cadence-ratio p95 2.47 | FAIL — heavy-file decode re-init stalls even same-file |
| | | **mitigation A/B**: alternating two video tracks (plan-prescribed probe), all variants | does NOT help: real p95 115.3 vs 108.0 ms; uniform pins to exactly 1.00 frame (compositor-locked delivery, zero dropped frames) | **mitigation rejected — §2.5 rule 1 (single track pair) stands** |
| 2 | Swap-blink = 0 on winner, 100 edit cycles, A/B vs mutate-in-place | real preresclip (4K-class 120-timebase) | A rebuild+preseek+swap: **27 stalls >250 ms, 0 black frames** (gap p50 169 / p95 300 ms); B mutate-in-place: 89 blinks, stalls ≥2 s (max 8.8 s) | winner **A**; **FAIL** on heavy real material |
| | | synthetic 1080p30 (`--fixture` leg) | A: **0 blinks, 0 black** (gap p50 80 / max 112 ms); B: 73 blinks | **PASS** — swap mechanics sound; the real-leg FAIL is decode spin-up, not the swap design |
...
### Watch-session checklist (Lillian — HARD STOP)

1. `cd mac/ClipFarmKit && swift run -c release n2harness demo` — watch the inbox + portrait + HLG assembly; **R** re-loads mid-play (swap-blink eyeball — decode-level numbers say ~80–110 ms gap on 1080p30, ~170–300 ms on the heavy file; judge whether it *reads* as a blink), **L** loops 1.5 s at the playhead (trim-loop feel), Space pause, Esc quit. Then `demo --real` (inbox files only).
2. Adjudicate the gate table — each row PASS / FAIL-pivot (D11) / REVISE-BUDGET (§6) / accept-as-limitation. The FAIL rows plus the cross-cutting read above are the decision packet.
3. Listen: `~/Library/Caches/ClipFarm-N2Gates/audio/fixture-fades-{on,off}.wav` and `real-fades-{on,off}.wav`.
4. Eyeball exports in QuickTime + VLC + Chrome: `~/Library/Caches/ClipFarm-N2Gates/export/` — `spike-a-passthrough.mov`, `spike-c-standard-reencode.mp4`, `spike-c-real-iphone.mov`, `spike-c-real-preresclip.mov`, `rotation-passthrough.mov`.
5. PNGs: `~/Library/Caches/ClipFarm-N2Gates/frames/` (rotation pillarboxing; hdrseam per-segment grabs).
6. If a real iPhone HDR (HLG) clip exists: drop it into `~/ClipFarm/Footage/` and re-run `swift run -c release n2harness seams real` + `hdrseam`.
7. N1's deferred checklist (its closeout, items 1–3) also runs at this stop.

**Test counts:** **149 green** (`swift test`): 117 carried baseline (118 − the deleted CFMedia smoke marker) + 18 CompositionPlanner/MediaTime pure tests + 14 probe/builder integration tests (tiny AVAssetWriter fixtures rendered at test time in temp — no footage dependency). `CFExportModule` marker + smoke test stay until N12 (the export spike is harness code by design — N1 delta 5).

**Files:** NEW `mac/ClipFarmKit/Sources/CFMedia/{MediaTime,AssetCache,MetadataProbe,CompositionBuilder,PlayerEngine}.swift`, `Sources/CFMediaTestSupport/{MediaFixtureRenderer,PixelProbe}.swift`, `Sources/n2harness/{Main,HarnessEnv,FrameTap,MP4BoxParser,GateSeams,GateBlink,GateRotationHDR,GateRebuildLoopStep,GateFades,GateExportSpike,Demo}.swift`, `Tests/CFMediaTests/{MediaFixtures,PlannerTests,MediaIntegrationTests}.swift`. MODIFIED `Package.swift` (CFMediaTestSupport + n2harness targets), `Sources/CFStore/LibrarySettings.swift` (+`smoothCutAudio`, own commit `8e24684`). DELETED the `CFMedia.swift` N0 marker + its smoke test.
