...
## Phase N3 — Ingest — native rewrite

**Manual verify: DEFERRED** (N3 tier: auto-continue) — checklist below; clears at the **N4 HARD STOP** (combined N3 + N4; N0/N1 already cleared at the N2 stop). Deferred-verification debt after this phase: 1.

**Built (2026-07-06):** point at a folder, get sources + clips — natively, with segmentation as a tunable setting (plan §4/N3). Plan entry committed pre-implementation (`da80e86`); work landed as six commits (schema v2 `b3c998e` → dependency+ffmpeg seam `e09e794` → CFDomain segmentation `9fe947a` → CFStore ingest/re-apply `6c12084` → CFMedia adapter/waveforms `c7ed5c4` → app shell `ed2557b`), plus the cold-review fix commit (dispositions below). **Mid-plan coordinator update honored:** Lillian moved the web-processed golden-master material into the inbox while the plan entry was being written — the golden-master leg was upgraded from "defer, N2-style" to **run**, before the plan commit.

### Headline result — the golden masters PASS on real data

- **Pre-flight (reference-side):** fresh Python segmentation over the three inbox sidecars reproduces the legacy `clipfarm.json` clips bit-for-bit (btc.0.2: 10, btc.0.4: 91, freestylingbtc0: 8; IDs re-derivable, texts equal, zero hand-edits) — so the committed legacy state is a valid oracle.
- **Swift vs web, domain level** (`realInboxSegmentationMatchesTheWebVersionsClips`): Swift segmentation (threshold 2.0, tail = word-end for comparability) over the real sidecars produces **identical clip IDs, identical `start_sec`/`end_sec` (bit-exact Double equality, no epsilon), identical `transcript_text`** for all 109 clips across the three sources, diffed against the legacy `clipfarm.json` loaded through the test-only fixture path (D9).
- **Swift vs web, end-to-end** (`ingestingTheRealInboxMatchesWebClipCounts`): `ingestFolder` over the actual `~/ClipFarm/Footage/` (read-only; temp library; stubbed probe so no AVFoundation churn on multi-GB files) yields per-source clip counts equal to the web version's, remaining videos footage-only, zero rejections.
- **Synthetic leg** (`syntheticGoldenMasterMatchesReferenceImplementation`): a committed fixture generated by `scripts/gen_segmentation_golden.py` running the *reference* implementation over 402 seeded words (deliberate exact-2.0s gaps — the `>=` boundary — plus .5 ms half-even-rounding timestamps) at two thresholds; Swift matches ranges, IDs, and texts exactly. Runs on any machine (test resource); the real-inbox legs skip visibly (`.enabled(if:)`) where the material is absent.

### What shipped, by module

**Schema v2 (own commit, `b3c998e`):** `sources.original_path TEXT NULL` — `.mkv` remux provenance (PROVISIONAL 1). `Source.originalPath` carried through entity/Codable (`original_path`)/record/fixtures. Version-stream unification: `ClipFarmState.currentVersion = 2` is the one constant (the backup carries the schema version so restores migrate — plan §2.3); `LibrarySchema.schemaVersion` mirrors it; the v1 migration now stamps a literal `'1'` and v2 updates the stamp to `'2'` (each migration stamps the version it produces — safe: no pre-existing native library anywhere; `~/ClipFarm/clipfarm.db` was first created by this phase's launch smoke, already at v2). Migration tests updated (`v99` is the new refuse-newer marker) + two new: `v1LibraryUpgradesToV2PreservingRows` (genuine `migrate(upTo: "v1")` then full), `originalPathRoundTripsThroughSourceRecord`.

**swift-subprocess + ffmpeg seam (`e09e794`):** dependency landed **pinned `.exact("1.0.0-beta.1")`** per Lillian's pre-approval (QUESTIONS.md → Answered; Apache-2.0), dependency of CFExport only; **both lockfiles re-committed** (Kit + xcodeproj workspace). `FFmpegLocator` (D16 seam — override path → `PATH` → Homebrew-ARM/Intel/MacPorts/system; a bad override is nil, never a silent fallback) + `MKVRemuxer` (D15: `ffmpeg -map 0:v? -map 0:a? -c copy -f mp4`, sibling `<stem>.mp4`, skip-if-exists, dot-prefixed temp + atomic rename so a killed run never leaves a trusted half-file, **streaming stderr** per D16 with a 20-line tail in the error). 7 CFExport tests including real-ffmpeg integration (lavfi-generated `.mkv` fixtures; `.enabled(if:)` skip when ffmpeg absent).

**CFDomain (`9fe947a`):** `Segmentation.rangesBySilence` — exact `segment_words_by_silence` port (`gap >= threshold` split, named test `exactThresholdGapSplits`); `Segmentation.applyTailPolicy` — D18's three modes over the silence ranges (ranges partition consecutive words, so "next word's start" ≡ next range's start): `extendToNextWordStart` (default; last clip → source duration when known, never shrinks on degenerate shorter-than-last-word durations), `fixedPadding` (clamped per PROVISIONAL 2), `wordEnd` (identity — web behavior, golden-master mode); `segment(...)` composite. `WhisperTranscript.transcriptText(from:to:)` — `transcript_text_for_range` port (half-open `[start, end)`, leading-space raw concat, Foundation-free strip — CFDomain still has zero dependencies). `SegmentationTailPolicy` **moved from CFStore to CFDomain** (the pure function needs it; CFStore re-exposes via its CFDomain import — all use sites were dot-shorthand, zero churn). `ProbedSourceInfo` value type added to CFDomain so CFMedia (producer) and CFStore (consumer) share it without a new module edge.

**CFStore (`6c12084`):** `Ingest.swift` — the `ingest.py` port as a **two-pass orchestrator**: pass 1 (async, zero writes) scans (extensions `{mov,mp4,m4v,mkv}`, case-insensitive, filename-sorted), hard-rejects `__` stems with the sanitized-rename offer, remuxes `.mkv` (failure → `remux-failed` rejection, PROVISIONAL 5), loads sidecars (`Sidecar.load`: unreadable/malformed/schema-version-mismatch split exactly as `_load_sidecar`; version checked before full decode), probes via the injected `SourceProbe`, and plans adds/upgrades/skips against both known-by-resolved-path and planned-in-this-run indexes (the `.mkv`+sibling-`.mp4` pair dedupes deterministically); pass 2 (sync, `@MainActor`) applies every write inside **one undo group "Ingest Folder"** (PROVISIONAL 4) — no `await` between undo registrations. Ported semantics intact: footage-only ingest with warning; sidecar-rejection still registers the source; re-ingest idempotent; transcript-appears-later upgrade (via the new undoable `updateSource` mutation + `LibraryStoreError.unknownSourceID`); duration policy sidecar → probe → nil; clip IDs skip-if-exists. Native: `fps` = probe `nominalFrameRate` (display only, N2 delta), `is_hdr`/`natural_*` recorded, segmentation reads `LibrarySettings` (threshold + tail policy + padding), FTS rows land via the N1 triggers (asserted). `ReapplySegmentation.swift` — D18's per-source action: **ID-preserving diff** (PROVISIONAL 3 — recomputed ranges whose encoded ID exists keep their rows, tags + `created_at` surviving; only genuinely changed boundaries delete+insert), `boundary_edited` clips never touched, explicit tag-row cleanup for deleted clips (FK order) **and `needs_review` on attempts referencing them** (the spec's delete-propagation rule applied forward of N5, with before-values captured for undo), snapshot (`re-apply-segmentation`) + named undo ("Re-apply Segmentation") both-always, true no-op → neither. 33 new CFStoreTests.

**CFMedia (`c7ed5c4`):** `SourceMetadata.probedSourceInfo` adapter (D17 → ingest fields; duration via the sanctioned `MediaTime.seconds`, non-finite/zero → nil). `WaveformService` (actor): AVAssetReader over the **first audio track only** (N2 lesson: spatial second tracks), LPCM Float32 interleaved, **vDSP peak per bucket** (`vDSP_maxmgv`) at 100 buckets/sec (plan's 50–100), carry buffer across sample-buffer edges, trailing partial bucket kept; binary cache `CFWV | version | bucketsPerSecond | count | Float32[]` (LE) at `<library>/cache/waveforms/<sourceID>.waveform`; skip-if-exists validates the file (truncated killed-run caches regenerate); **in-flight `Task` memoization** (the N2 fixture-actor reentrancy lesson); decode is `@concurrent` (off the actor's executor). No-audio sources get an **empty (0-bucket) file** — "no audio" is distinct from "not generated yet" (no file), N11's degrade contract. **Release perf on real material: btc.0.2.mov (181 s) → 18,134 buckets in 0.26 s (~690× realtime)**; the same run in debug is 3.9 s — the N2 20–50× debug-loop warning holds, measured in release per the delta.

**App shell (`ed2557b`):** `AppStore` (`@MainActor @Observable`, created in the App struct, injected via `Environment` — no globals): opens `~/ClipFarm/` (D28) on first appearance with the **window's `UndoManager`** (`@Environment(\.undoManager)` — the system Edit menu drives store undo with zero extra wiring), **creates the footage inbox `~/ClipFarm/Footage/` on first run** (D34), wires the probe closure to `MetadataProbe`/`probedSourceInfo` and remux to `MKVRemuxer`, kicks waveform generation **after** ingest returns (never blocking), refetch-based source list (ValueObservation read models are N4's job as planned). Library page: `NSOpenPanel` **defaulting to the inbox** + drag-a-folder-in (`dropDestination`), empty-state explaining the sidecar convention + inbox path, source rows (filename, duration, fps, HDR/footage-only/remuxed-from-mkv badges, clip count, unavailable dimmed), per-source **Re-apply Segmentation** context action, ingest status capsule (summary + rejections). Settings page: the D18 segmentation section (threshold slider 0.25–5.0 s, tail-policy picker, padding slider when fixed-padding) writing through `updateLibrarySettings`, with copy stating re-apply semantics. Views mutate only through store methods.

### PROVISIONAL calls (all 5 logged in QUESTIONS.md → Open)

1. `.mkv` provenance → `sources.original_path` column (the plan-promised "reviewable default"; schema §2.3 amendment).
2. Fixed-padding tail clamp → next clip's first-word start AND source duration.
3. Re-apply semantics → ID-preserving diff (tags survive unchanged ranges).
4. Ingest undo granularity → one grouped "Ingest Folder" step.
5. Failed `.mkv` remux → hard reject (`remux-failed`), mirroring the `__`-stem shape.

### Deviations from the committed plan entry

1. **Version-stream unification** (`ClipFarmState.currentVersion`, v1 stamps literal `'1'`): the plan entry named the v2 migration but not the constant unification; done so the schema version and backup-shape version can't drift apart. Affected three existing tests (updated, incl. the future-marker rename to `v99`).
2. **`SegmentationTailPolicy` relocation to CFDomain** — implied but not spelled out in the plan entry.
3. **Waveform empty-file semantics for no-audio sources** — chosen (vs. writing no file) so N11 can distinguish "no audio" from "pending"; recorded here rather than as a sixth PROVISIONAL (it's a cache-file detail, not product behavior).
4. **Two `print` diagnostics** on the app's library-open path (path + failure) — left in deliberately as launch breadcrumbs until N4's real Library lands.
5. The reference's `test_ffprobe.py` (9 tests) ports as **semantics, not parsing**: r_frame_rate parsing is obsolete by D17; the surviving behaviors (probe failure → nil fields, source still ingests; duration fallback order) are covered in `IngestTests` + the adapter tests. Recorded per the §3 adjudication rule.

### Cold review (2026-07-06) — findings & dispositions (review 2 of 2)

Reviewer ran with zero implementation context. 11 findings: **9 ACCEPTED and fixed same-day** (fix commit follows the closeout commit), **2 ACCEPTED AS NOTED** (deferred with named owners). Full suite re-run after fixes: **232 green** (+4 regression/behavior tests).

1. **[MAJOR] Re-apply throws an obscure `DatabaseError` on a pure-insertion diff** (zero deletions, ≥1 insertion — reachable at N5 the moment every clip on a source is hand-corrected and a setting changes; `placeholderList(0)` emitted `"(?)"` against zero bound arguments) → **ACCEPTED** — the sharpest catch: a thrown raw DB error before any mutation, no snapshot/undo, no test coverage. Fixed: the undo-capture read is skipped entirely when `deletedIDs.isEmpty`; `placeholderList` now `precondition`s `count > 0` instead of emitting a mismatched placeholder (every call site guards empties). New regression test `insertionsOnlyDiffAppliesCleanlyAndUndoes` (all clips `boundary_edited` + tail-policy flip → applies, snapshots, undoes/redoes).
2. **[MINOR] Ingest scan lists hidden files** (crash-orphaned `.stem.remux-*.mp4` temps and AppleDouble `._*.mov` litter would ingest as garbage footage-only sources; the remuxer's "never picked up by a folder scan" contract was false as consumed) → **ACCEPTED**. Fixed: `.skipsHiddenFiles` on the ingest scan — which also makes the remuxer's documented invariant true. New test `hiddenFilesAreNeverScanned`.
3. **[MINOR] Skip-if-exists silently adopts any same-stem sibling `.mp4` as "an earlier remux"** (an unrelated pre-existing `x.mp4` would become the mkv's playable file with silently-wrong provenance) → **ACCEPTED**, proportionate fix: the adoption is now recorded in `IngestResult.warnings` ("adopted existing sibling … if it is an unrelated file, delete or rename it and re-ingest"), emitted once at first registration — not on idempotent re-ingests. Heuristic sanity checks (mtime/duration) deliberately not added: a deterministic always-warn beats a probabilistic sometimes-warn for a case the user resolves by deleting one file. New test `adoptingAPreexistingSiblingMP4IsWarnedNotSilent`. *Confirmation-round residual (reviewer, not a regression — recorded as an open item per the max-two-rounds rule):* if the sibling `.mp4` was ingested as its own source in an **earlier** run and the `.mkv` arrives later, no warning fires — the mkv appears only in `sourcesSkipped`, its content never ingested, no `original_path` recorded. Logged in `QUESTIONS.md` → Open; earliest natural owner N4's Library/ingest UX.
4. **[MINOR] Undo wiring one-shot; a momentarily-nil environment `undoManager` silently disables store undo for the session** → **ACCEPTED**. Fixed on both sides: `LibraryStore.adoptUndoManager(_:)` (first adoption wins; `undoManager` is now `private(set) var`, read at every registration) + `AppStore.openDefaultLibraryIfNeeded` adopts into an already-open store + `RootView.onChange(of: undoManager)` re-binds + a WARNING breadcrumb when opening with nil. New test `lateUndoManagerAdoptionCoversSubsequentMutations`.
5. **[MINOR] Completed waveform generations stay memoized in `inFlight` forever** (a deleted/truncated cache or repointed source couldn't regenerate until app restart) → **ACCEPTED**. Fixed: the memo now covers only the in-flight window (`defer` clears on success AND failure); the cache-file check serves subsequent calls. Existing round-trip test extended to same-instance regeneration after truncation and after deletion.
6. **[MINOR] Informational no-op reported through the error channel** (orange "Re-apply: no boundary changes…" styled like a failure; info-as-error could mask a real prior error) → **ACCEPTED**. Fixed: `AppStore.lastActionInfo` (secondary styling in the status capsule); re-apply now reports both the changed summary and the no-op case through it; ingest clears both channels at start.
7. **[NIT] `nominalFrameRate == 0` recorded `fps: 0.0` instead of nil** → **ACCEPTED**. Fixed: `flatMap` + `> 0` guard in the probe adapter ("0 fps" is unknown, not a rate). No dedicated test: `VideoTrackInfo` has no public memberwise init to fabricate a 0-rate track, and the real-fixture adapter tests cover the positive path.
8. **[NIT] Cache-format comment claimed "all little-endian" but the Float32 payload is a native-endian memcpy** → **ACCEPTED**, comment narrowed (header ints explicitly LE; payload native-endian — LE on every supported platform; local cache, not interchange; version-bump note added).
9. **[NIT] Closeout said "five commits" — it's six** → **ACCEPTED**, corrected above with the hashes.
10. **[NIT] Settings sliders write all four settings rows per drag tick** → **ACCEPTED AS NOTED, no code change**: harmless at four rows/personal scale; **owner: N7** (the phase that grows the Settings surface) picks up debouncing if it matters then.
11. **[NIT] Ingest pass 1's synchronous file I/O (scan + sidecar double-parse) runs on the MainActor** → **ACCEPTED AS NOTED, no code change**: fine at N3 scale per the reviewer; **owner: N4** (which reworks AppStore reads onto ValueObservation anyway) — a `nonisolated` pass 1 is the natural refactor there.

### Environment findings (for the N4 session and Lillian's verify)

- **Stale-instance confound:** a pre-N3 ClipFarm (PID 51307) from Lillian's Xcode session sits **suspended under the debugger** (`STAT=SX`) — it ignores signals/AppleEvents until resumed. While it exists, freshly launched instances' windows self-close ~5–10 s after launch (reproduced on the unmodified N0-baseline app too — **not an N3 regression**; the library open itself works: the real `~/ClipFarm/clipfarm.db` was created at schema v2 with `original_path` present, verified via `sqlite3`). Before verifying: **stop the old Xcode debug session** (releases 51307), then launch normally. Implementer note: I sent that PID kill signals before realizing it was her suspended session — they'll land whenever it resumes; nothing of value is lost (placeholder-era app, no state), but it was her process and is flagged here.
- **Two DerivedData directories exist**; `ClipFarm-dzsggxcpzdvnckfgvjjmixtxvsrb` is a stale pre-repo-move build (binary dated 01:28) that cost this phase an hour of misdiagnosis — safe to delete; the live one is `ClipFarm-evnssetyqxmfyefwuavdrlclsyiw`.
- No TCC prompt fired for `~/ClipFarm/` reads in any test or launch (not a protected folder category), as predicted.

### Manual verify — DEFERRED checklist (runs at the N4 hard stop)

1. Quit the stale Xcode debug session first (see above), then launch ClipFarm → Library → "Ingest Folder…" (defaults to `~/ClipFarm/Footage/`) → expect 8 sources: btc.0.2 (10 clips), btc.0.4 (91), freestylingbtc0 (8), five footage-only (0 clips) — counts equal the web version.
2. Note any TCC prompt (none expected for `~/ClipFarm/`).
3. Flip tail policy in Settings → "Re-apply Segmentation" on one transcribed source → (with N4's click-to-play) **hear that clips no longer feel cut short** under the default extend-to-next-word-start policy; Cmd+Z restores.
4. `~/ClipFarm/.snapshots/` gains a `re-apply-segmentation` snapshot; `sqlite3 ~/ClipFarm/clipfarm.db 'PRAGMA table_info(sources)'` shows `original_path`; `cache/waveforms/` fills in post-ingest.
5. Golden-master eyeball is optional — automated twice over (domain diff + end-to-end counts).

### Next-phase delta (N4 — plan §4/N4 read in full; plan entry amended)

1. **Sidecar loading already exists in CFStore** (`Sidecar.load`, typed rejections, schema-version gate) — N4's `transcripts.py` port should wrap THIS seam with the mtime-keyed cache, not re-implement validation. The N4 plan entry's "whisper sidecar validation" is substantially done.
2. **Real data is one UI click away**: the inbox holds three web-processed transcribed sources + five footage-only; ingest via the Library UI populates the real library (109 clips). N4's "browse btc.0.4" verify has its material ready, and FTS rows are already written at ingest (tested) — N4's search work is read-side UI only.
3. **AppStore exists** (minimal, refetch-based). N4 swaps reads to GRDB `ValueObservation` per plan; the Environment injection + window-UndoManager wiring is established. N4's Library rebuild must preserve the N3 affordances: ingest toolbar/drop, per-source re-apply, footage-only badge (already there), and the Settings segmentation section.
4. **Environment cleanup before the hard stop** (see Environment findings): stale suspended instance + stale DerivedData dir; the CLI "launch the built product binary directly" loop must use the `evnss…` DerivedData path.
5. **Waveform strips are ready for consumers** at `<library>/cache/waveforms/<sourceID>.waveform` (`WaveformService.read`); N4 doesn't need them (N11 does), but the inspector pane shouldn't preclude a strip slot.
6. **mac/CLAUDE.md amendment effective N4**: implementer/reviewer may each run ONE read-only `Explore` retrieval helper (never for binding documents).

**Test counts:** **232 green** after the cold-review fixes (`swift test`, ~4 s): 156 baseline + 76 new — CFDomain 78 (was 51: +25 segmentation/tail/text + 2 golden masters), CFStore 95 (was 58: +2 migration/round-trip, +23 ingest incl. hidden-files/sibling-adoption/late-undo-adoption, +9 re-apply incl. the insertions-only regression, +2 update-source, +1 FTS-at-ingest), CFMedia 46 (was 40: +2 probe-adapter, +4 waveform incl. the real-material perf leg), CFExport 7 (was 1: +6 locator/remux), CFLLM 6 (unchanged). Pre-review count was 228; the cold review added 4. Golden masters and real-material legs skip visibly on machines without ffmpeg or the inbox.

**Files:** NEW `Sources/CFDomain/{Segmentation,ProbedSourceInfo}.swift`, `Sources/CFStore/{Ingest,ReapplySegmentation}.swift`, `Sources/CFExport/{FFmpegLocator,MKVRemuxer}.swift`, `Sources/CFMedia/{ProbeAdapter,WaveformService}.swift`, `mac/ClipFarm/App/AppStore.swift`, `Tests/CFDomainTests/{SegmentationTests,SegmentationGoldenTests}.swift` + `Resources/segmentation-golden.json`, `Tests/CFStoreTests/{IngestTests,ReapplySegmentationTests}.swift`, `Tests/CFMediaTests/WaveformAndProbeAdapterTests.swift`, `Tests/CFExportTests/RemuxTests.swift`, `scripts/gen_segmentation_golden.py`. MODIFIED `Package.swift` (+swift-subprocess exact-pin, CFExport product dep, CFDomainTests resources), both `Package.resolved` lockfiles, `LibrarySchema.swift` (v2), `Entities.swift`/`EntitiesCodable.swift`/`Records.swift`/`Fixtures.swift` (originalPath), `ClipFarmState.swift` (currentVersion), `LibrarySettings.swift` (enum moved out), `LibraryStore.swift` (+unknownSourceID), `StoreMutations.swift` (+updateSource), `MigrationTests.swift`/`StoreOpenTests.swift`/`StoreRoundTripTests.swift`, app `ClipFarmApp/RootView/LibraryView/SettingsView` + pbxproj (xcodebuild's own synchronized-group normalization). The `CFExportModule` marker + smoke test remain per the kickoff instruction; `n2harness` untouched.
