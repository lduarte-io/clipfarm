...
2. **HDR↔SDR seam (gate 4): DEFERRED — not accepted, not failed.** Lillian will record a real iPhone HDR clip and drop it in the inbox; the re-run (`seams real` + `hdrseam`) decides. Stays on the watch-session checklist.
...
1. **Watch the fixed demo** (the black window is fixed — two root causes: the AVPlayerLayer was never reliably attached/sized [now it IS the view's backing layer], and the CLI run loop starved the main dispatch queue [now a hybrid pump]; the self-check output proves layer readiness, video rect, and real-time playback programmatically — what remains for eyes is only "does it look right"):
   `cd mac/ClipFarmKit && swift run -c release n2harness demo`
   Expect: video visible immediately (first segment is h264-4k-vfr pillarbox-fit); the `DEMO SELF-CHECK` lines at 2 s/5 s should say `layerReadyForDisplay=true`. Keys: Space pause, **R** live-reload (swap-blink eyeball — ~80–110 ms gap expected on light material, ~220–325 ms freeze-frame on the heavy 4K-VFR file, per the accepted limitation), **L** 1.5 s loop at the playhead, Esc/Q quit. Click the window once if keys don't respond. Then `demo --real` for the inbox-only assembly.
...
4. **HDR re-run — the one OPEN gate row (adjudication 2):** when you have a real iPhone HDR (HLG) clip, drop it into `~/ClipFarm/Footage/` and run `swift run -c release n2harness seams real` and `swift run -c release n2harness hdrseam`. Reports land in `~/ClipFarm/outputs/reports/`; the HDR row adjudicates on those numbers.
