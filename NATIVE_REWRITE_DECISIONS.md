...
### D20 — Transcript view — **Status: LOCKED for ClipFarm (explainer recorded for the writing app)**
**Pick:** STTextView (TextKit 2) wrapped in NSViewRepresentable; raw NSTextView as fallback if its word-hit-testing model fights us at N4.
**Round-2 addendum (one-maintainer risk / swap cost — Lillian's question):** STTextView is contained behind a single **`TranscriptViewAdapter` seam we own** (operations: set attributed content, word hit-test at point, apply/clear highlight ranges, selection callbacks, scroll-to-word). Nothing outside that one wrapper file may import or reference STTextView. Swap cost is therefore re-implementing the adapter on raw NSTextView/TextKit 2 — a few days, mostly highlight plumbing — with zero changes anywhere else in the app. Additional insurance: the dependency version is pinned (upgraded deliberately, never floating), and the MIT license permits vendoring the source into the repo if the project ever goes quiet. The NSTextView fallback implements the same adapter contract.
...
| D20 | Transcript view | STTextView behind a swap-cheap adapter seam; NSTextView fallback | LOCKED |
