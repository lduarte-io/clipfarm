...
*Product session (2026-07-11, Fable + Lillian, pre-N4): the five N3 PROVISIONALs answered (all keep-as-implemented); D35's deferred generation-timing items decided; **D36–D40 added** (trim-mode edit-point semantics; assembly outline shape A + Duplicate Project; no-sync; N12 staged-split pre-authorization; Track 1.5 phases N13.5/N13.6). Session record: `PRODUCT_SESSION_2026-07-11.md`.*
...
### D35 — Proxy workflow for heavy sources — **Status: LOCKED (drafted at the N2 adjudication 2026-07-06; ratified by Lillian the same day, as drafted)**
**Why it exists:** N2's gates measured every latency FAIL to one root — first-frame decode spin-up on heavyweight material (4K-class, high-rate VFR, long-GOP H.264) after any discontinuity: seam entry ~68–130 ms, edit-swap stalls >250 ms on 28/100 cycles, trim-loop restart ~87–192 ms vs the 50 ms budget. The same operations on 1080p30 material pass every gate. Lillian adjudicated the FAILs ACCEPTED AS LIMITATION — no D11 pivot, no §6 relaxation — and proposed this as the designated fix.
**Pick:** an optional per-source **editing proxy** — re-encode a lighter copy (e.g. 1080p30 H.264/ProRes-proxy-class) of any heavy source; preview, trim-loop, and assembly run against the proxy; export applies the **identical resolved cut list** to the original. WYSIWYG is preserved by construction for cuts and timing (cut lists are times, not pixels); the proxy trades only preview fidelity, opt-in per source. Owner: **N12** (the resolver already feeds preview and export from one range list; proxies swap preview *material*, never cut math) — PROVISIONAL placement; proxy *generation* timing (N12 vs an N3-style post-ingest background job) is decided when N12 lands.
**Rejected at adjudication:** D11 pivot to the sample-buffer pipeline (the composition architecture passes everything on the footage profile ClipFarm targets); relaxing the §6 budgets (they stay as the bar the proxy path must meet on heavy sources).
**Generation timing + proxy rules (Lillian, 2026-07-11 product session — the items the decision text deferred to N12, settled early):** generation is **both paths** — an on-demand offer at first edit of a heavy source + a per-source "Generate Proxy" action, PLUS a Settings toggle "auto-generate proxies for heavy sources at ingest," **default OFF** (no silent multi-GB background encodes). Both paths share the detection heuristic, generator, and cache. Proxy profile is **1080p-class** (run-6 evidence: 4K all-intra stalls too — resolution is the lever, not codec alone). **Hard rules:** cut math, frame-nudge increments, and keyframe ticks / snap-to-keyframe ALWAYS derive from the ORIGINAL file (`KeyframeMapService` reads originals only — proxy keyframes land elsewhere; snapping to them would silently break the Lossless-tier guarantee); the proxy swaps preview *material* only; export never touches proxies.

### D36 — Trim-mode edit-point semantics — **Status: LOCKED (Lillian, 2026-07-11 product session)**
**Resolves the N2 gate-6 fencepost as design, not accepted quirk.** (a) **Parked exactly on a cut, the monitor shows the INCOMING clip's first frame** — the first frame that survives the cut — matching NLE convention (Premiere/Avid/Resolve) and the domain's half-open `[s, e)` ranges. The platform naturally shows the outgoing frame, so PlayerEngine biases exact-boundary display to the incoming side (small accommodation, lands with N11). (b) **Nudge undo coalesces per burst**: nudges on the same edge coalesce until ~1 s pause or switching edges; one Cmd+Z returns to the burst start. Rejected: outgoing-frame display (platform default, backwards from convention and from `[s, e)`); per-tap undo (floods history); per-trim-session undo (Cmd+Z nukes all trim work on the clip).

### D37 — Assembly outline (matching script ≠ assembly outline), shape A — **Status: LOCKED (Lillian, 2026-07-11 product session)**
**Why it exists:** the project's script currently plays two roles — the *matching target* (fuzzy string-matching finds takes of each line) and the *assembly outline* (the TOC you pick takes from). They coincide for the scripted-takes wedge; they diverge for "make a video arguing X from everything I've recorded" — outline beats never said verbatim can't be string-matched (that's semantic/LLM territory).
**Pick — shape A, built in Track 1:** the assembly outline is an **ordered list of beats, one outline per project**; each beat is either a **script line** (string-matched takes, as today) or a **free-text intent** (reuses the ad-hoc tag machinery — the LLM assigns clips to it semantically in the same tagging run, same validation). Absent an explicit outline, the default outline = script lines in order (wedge unchanged). Lands as amendments to N6 (brief format) / N7 (tagging) / N8 (TOC rendering) / **N9 (extension, same session): premade strategies follow the outline** — best-per-beat in outline order, free beats contributing their top semantically-tagged clip, no-clip beats skipped; the no-explicit-outline default (script order) keeps the N9 golden masters valid. N10's replace-picker generalizes to same-beat siblings. Companion affordance, Track 1.5 (D40): **Duplicate Project** — clones brief/outline/tags with zero re-mine, making "try a different structure" instant, because a different structure IS a different project (core principle 4 working as designed).
**Rejected — shape B (outlines as first-class objects, multiple per project):** would triple-nest project→outline→attempt and force premades, the take grid, and attempts to become outline-relative (an N9–N10 ripple, not just schema). Shape A migrates additively into B later (the single outline becomes "outline #1") if real usage ever demands it — nothing is walled off.

### D38 — No multi-machine sync — **Status: LOCKED (Lillian, 2026-07-11 product session)**
**Pick:** single-machine library + local footage inbox, **on record as deliberate** (not an oversight). Pro video tools are single-machine by norm; SQLite + media don't cloud-sync safely (the 2026-07-06 iCloud incident is the local evidence); `File → Back Up Library…` / restore (N13) covers machine moves. Multi-machine sync stays in Future Ideas with no Track 2 commitment.

### D39 — N12 staged-split pre-authorization — **Status: LOCKED (Lillian, 2026-07-11 product session)**
**Contingency authority, not plan-of-record.** If N12 balloons mid-phase, the coordinator may split without stopping to ask: **N12a** = Standard-tier re-encode + export mode picker + post-export verification (fully WYSIWYG — Standard re-encodes audio, so micro-fades are honored trivially; real videos can ship from N12a) → **N12b** = Lossless passthrough + hybrid writer + D35 proxies, immediately after. The full phase remains the target.

### D40 — Track 1.5: product-iteration phases between the tracks — **Status: LOCKED (Lillian, 2026-07-11 product session)**
**Why:** Track 1 invests heavily in engine/export fidelity while the AI-quality layer — the pain the product exists to kill ("watching the same thing over and over") — gets a faithful port but no scheduled iteration; and the re-mining engine lacks a low-friction query entry point.
**Pick:** two phases after N13, before Track 2 (ordering vs N14 decided when Track 1 closes; N14 keeps its first-in-Track-2 commercial-gating priority):
- **N13.5 — Quality iteration**: dedicated tuning of premade/take quality with the full loop available. Levers: take-detection tolerance sets, filler/restart detection, LLM-ranked take ordering within lines, tagging batch-size/quality experiments, **audio-energy analysis (promoted from the N19 grab-bag — the N3 waveforms already carry loudness; `energy_shift` stops proxying energy with words-per-second)**. **Early-pull trigger:** if the N9 or N10 hard stop shows premades are mediocre, N13.5 pulls forward immediately.
- **N13.6 — Ask the library + Duplicate Project**: natural-language query over the whole library (query → LLM/semantic match over transcripts + tags → ranked clips with provenance → optionally save as a project) as the low-friction entry point to the re-mining engine; plus the D37 Duplicate Project action.
...
| D34 | Footage inbox | `~/ClipFarm/Footage/`, outside cloud sync; managed working folder | LOCKED |
| D35 | Heavy-source proxies | 1080p-class editing proxy; on-demand + auto-toggle (default OFF); originals own cut math + keyframes | LOCKED |
| D36 | Trim-mode edit points | Incoming frame parked on a cut; per-burst nudge undo | LOCKED |
| D37 | Assembly outline | Shape A: one outline/project, beats = script lines or free-text intents; Duplicate Project affordance | LOCKED |
| D38 | Sync | None — single-machine on record; backup/restore covers moves | LOCKED |
| D39 | N12 contingency | Staged split pre-authorized (N12a Standard-first → N12b) | LOCKED |
| D40 | Track 1.5 | N13.5 quality iteration (early-pull trigger) + N13.6 ask-the-library | LOCKED |
